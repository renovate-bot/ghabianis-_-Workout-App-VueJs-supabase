      ...
      >
        ...
        <div v-if="data.workoutType === 'strength'" class="flex flex-col gap-y-4 w-full">
          <div
            ...
            v-for="(item, index) in data.exercices"
            :key="index"
          >
            <div class="flex flex-2 flex-col md:w-1/3">
              ...
              <input
                ...
                class="p-2 w-full text-gray-500 focus:outline-none"
                type="text"
                v-model="item.exercices"
              />
              <p v-else>{{ item.exercices }}</p>
            </div>
...
export default {
  ...
  setup() {
    ...
    const deleteExercise = (id) => {
      if (data.value.exercices.length > 1) {
        data.value.exercices = data.value.exercices.filter(
      ...
      try {
        const { error } = await supabase
          ...
          .update({
            ...
            exercices: data.value.exercices,
          })
          .eq("id", currentId);
        if (error) throw error;
        edit.value = false;
